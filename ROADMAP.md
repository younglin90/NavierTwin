...
## 현재 단계: v5.0~v5.6 — 일반 CFD 트윈 플랫폼 전환 ✅ (2026-07-18 완료)
...
남은 것은 스코프가 아직 확정되지 않은 대형 항목뿐 — 아래 "다음 단계" 참조.
...
## 다음 단계 — 스코프 확정 필요(제품 설계 결정 선행)

아래는 외부 검토(§6½)가 언급했지만 이번 자율 실행 배치에서 다루지 않은
항목이다 — 코드 몇 파일로 스코핑할 수 없고 방향 결정(어떤 걸 먼저/얼마나
깊게)이 우선 필요하다:

- **대형 3D/HPC 스케일링** — 지금까지의 데모/검증은 소~중규모(수백~수천
  점)다. 실제 산업 규모 3D(수백만 셀) 케이스 세트에서 Route 1/2 전략이
  버티는지, MPI 배치 CLI 를 실제 클러스터 잡과 어떻게 엮을지는 사용자
  데이터로 실측해야 판단 가능.
- **운영형 디지털 트윈(단계 6)** — 센서 동기화·자료동화·온라인 보정. 현재
  플랫폼은 "대리모델·연산자 학습/검증 플랫폼"(검토 §6½ 재프레이밍)이고,
  이건 완전히 다른 제품 층(실시간 데이터 파이프라인, 신뢰성 요구사항)이라
  별도 기획이 필요.
- **모델 등급제의 Domain-Specific/Experimental 확장 카탈로그**
  (UPT/DoMINO/AB-UPT/Transolver++) — 우선순위는 실사용 데이터 특성에
