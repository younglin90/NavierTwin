...
References:
    Manohar et al., "Data-Driven Sparse Sensor Placement for Reconstruction:
...
def select_sensors(
    ...
    # QR 분해로 피벗 열 선택 (scipt.linalg.qr with pivoting=True)
    try:
        from scipy.linalg import qr  # type: ignore[import-untyped]

        _, _, piv = qr(basis.T, pivoting=True)
    except ImportError:
        # scipy 없으면 numpy 그리디 대리 (근사)
...
def _greedy_pivot(
    ...
    for _ in range(min(n_sensors, n_modes)):
    ...
    # 나머지는 미선택 중 임의로 채움
    remaining = [i for i in range(n_space) if i not in selected]
