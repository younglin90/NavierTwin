"""MeshGraphNets 케이스 세트 트윈 엔진 — 재샘플 없는 원본 메쉬 위 예측 (Route 2, 3번째).

:class:`~naviertwin.core.gnn.meshgraphnets.case_set_mgn.CaseSetMGN` 을 웹/
데스크톱 트윈 계약(``predict(params)`` + ``training_metadata`` + ``save/load``)
에 맞춰 감싼다. :mod:`~naviertwin.core.digital_twin.mesh_gnn_engine`
(``MeshGNNTwinEngine``, GCN)과 나란한 구조이지만, 내부 모델이 진짜
``edge_features``(Δ좌표, ‖Δ‖)를 메시지패싱에 쓰는 MeshGraphNets 라는 점이
다르다 — ``predict_at`` 이 저장 케이스와 좌표가 일치하면 그 edge_index **와**
edge_attr 를 함께 재사용하고, kNN 폴백도 edge_attr 를 다시 계산해 넘긴다.

앱 배선의 핵심은 ``mesh_gnn_engine`` 과 동일하게
``training_metadata["varying_mesh"] = True`` 다: ``app.predict`` 의 기존
형상 가변 분기가 이 표시를 보고 ``service.predict_to_mesh(engine, μ, 보고
있는 케이스)`` 를 부르고, 그 함수는 ``engine.model.predict_at(coords, params)``
덕타이핑만 요구한다. 앱 예측/뷰어 경로 수정은 없다.
"""
...
class _MGNCaseSetModelFacade:
    ...
    def __init__(self, engine: "MGNCaseSetTwinEngine") -> None:
        self._engine = engine
        self.is_fitted = True
    ...
    def predict_at(
        ...
        edge_index/edge_attr/노드 피처를 재사용하고, 불일치하면 scipy
        cKDTree kNN 그래프(+ 재계산 edge_attr)로 폴백한다(메쉬 위상과 다른
        연결이 생길 수 있어 경고 로그).
...
class MGNCaseSetTwinEngine:
    """케이스-세트 MeshGraphNets 트윈 — 정상(steady) 파라미터 스윕 전용.
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            operator: fit 완료된
                :class:`~naviertwin.core.gnn.meshgraphnets.case_set_mgn.CaseSetMGN`.
            cases: 케이스별 그래프 정보 목록 — 각 항목은
                ``{"points": (N,3), "edge_index": (2,2E), "edge_attr": (2E,e),
                "base_x": (N,f)}``. ``base_x`` 는 μ 채널을 뺀 노드 피처
                (정규화 좌표+입력 필드). ``edge_attr`` 는 MGN 이 실제로
                쓰는 메시지패싱 에지 피처(Δ정규화좌표, ‖Δ‖) — mesh_gnn(GCN)
                엔진과의 핵심 차이.
            train_params: (n_cases, k) 학습 운전조건.
            param_names: 운전조건 이름 k 개.
            field_names: 학습 요청 필드 (예: ``["p", "U"]``).
            target_names: 채널 전개된 타깃 이름 (예: ``["p","U_x","U_y","U_z"]``).
            norm: :func:`~naviertwin.core.gnn.case_graph.graph_norm_from_cases`
                정규화 상수 (μ 정규화 재사용).
        ...
        if not bool(getattr(operator, "is_fitted", False)):
            raise RuntimeError("CaseSetMGN 을 먼저 fit() 해야 합니다.")
        ...
        self.model = _MGNCaseSetModelFacade(self)

        # TwinEngine 덕타이핑 (save_engine/_restore_engine 경로 호환).
        self.reducer_type = "mgn_case_set"
        self.surrogate_type = "case_set_mgn"
        self.model_type = "mgn_case_set"
        self.n_modes = 0
        ...
        self.training_metadata: dict[str, Any] = {
            ...
            "reducer": "mgn_case_set",
            "surrogate": "case_set_mgn",
    ...
    def _knn_base(self, coords: NDArray[np.float64]) -> dict[str, Any]:
        """미지 좌표용 kNN(k=8) 그래프 + edge_attr 폴백 — 위상 왜곡 가능성을 경고한다.
    ...
    def _predict_rows(
        ...
        for row in rows:
            graph = self._graph_for(base_x, edge_index, edge_attr, row)
    ...
    def _predict_at(
        ...
        """파사드 :meth:`_MGNCaseSetModelFacade.predict_at` 구현."""
        if not self.is_fitted:
            raise RuntimeError("MGNCaseSetTwinEngine: operator 가 fit 되지 않았습니다.")
        ...
        outputs = self._predict_rows(base_x, edge_index, edge_attr, rows)  # (n_rows, C×N)
        if single:
            return outputs[0]
    ...
    def predict(self, params: NDArray[np.float64]) -> NDArray[np.float64]:
        ...
        rep = self._cases[0]
        outputs = self._predict_rows(
            rep["base_x"], rep["edge_index"], rep["edge_attr"], rows
        )
    ...
    @classmethod
    def load(cls, path: str | Path) -> "MGNCaseSetTwinEngine":
        """저장된 MeshGraphNets 케이스 세트 트윈 엔진을 복원한다."""
        with Path(path).open("rb") as f:
            engine = pickle.load(f)
        if not isinstance(engine, cls):
            raise TypeError(f"MGNCaseSetTwinEngine 파일이 아닙니다: {path}")
        return engine
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"MGNCaseSetTwinEngine(n_cases={self._train_params.shape[0]}, "
            f"channels={self.target_names}, status={status})"
        )


__all__ = ["MGNCaseSetTwinEngine"]
