...
이 모듈은 Qt/torch 에 의존하지 않는다 — 순수 판정 로직이다.
"""
...
STRATEGIES: tuple[StrategySpec, ...] = (
    ...
    StrategySpec(
        ...
        needs_identical_mesh=True,
        needs_uniform_grid=False,
        supports_case_sets=False,  # ParametricDMD 배선 시 True 로 (로드맵 v5.2)
        supports_time_in_sweep=False,
        single_case_needs_steps=4,
        min_snapshots=4,
        note="시간 전이 규칙을 학습해 학습 구간 밖까지 예보 — 단일 케이스 시계열 전용.",
    ),
    StrategySpec(
        key="operator",
        name="신경 연산자 (FNO)",
...
def profile_data(
    ...
    if len(cases) > 1:
        ...
        if len(counts) > 1:
            ...
            try:
                from naviertwin.web.service import meshes_are_identical

                identical = meshes_are_identical(cases)
...
def _check(spec: StrategySpec, p: DataProfile) -> tuple[bool, str]:
    ...
    if p.n_cases > 1:
        if not spec.supports_case_sets:
            if spec.key == "dynamics":
                return False, (
                    "케이스 세트의 동역학 예보(ParametricDMD)는 아직 미지원 — "
                    "단일 케이스 시계열에서 쓰세요."
                )
