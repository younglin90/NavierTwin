...
class NavierTwinWebApp:
    ...
    def _init_state(self) -> None:
        ...
        # 카드와 무관하게 하나만 뜨던 nt_method_hint 를 보완해, 비정상을
        # 지원하는 전략들은 (μ, t) 로 함께 학습된다는 요약 배지를 띄우는
        # 데 쓴다.
        ...
        # Model — 메쉬 GNN (mesh_gnn, Route 2 · 케이스 세트 전용). 케이스마다
        # 자기 메쉬를 그래프로 직접 학습한다 — 공통 격자 재샘플이 없어 진짜
        # 구멍(형상 가변)이 그대로 보존된다. 예측은 varying_mesh 경로로 보고
        # 있는 케이스 메쉬 위에 표시된다.
    ...
    def _build_twin_dispatch(self) -> None:
        ...
        if method == "mesh_gnn":
            if self.state.nt_case_mode:
                # 케이스 세트 → 메쉬 네이티브 GNN (Route 2, 재샘플 없음).
                self._build_mesh_gnn_twin()
                return
            self._fail(
                "메쉬 GNN",
                RuntimeError(
    ...
    def _build_mesh_gnn_twin(self) -> None:
        """②Model — 케이스 세트에서 메쉬 네이티브 GNN(mesh_gnn) 트윈을 학습한다.

        정상 파라미터 스윕 전용 (Route 2) — 케이스마다 자기 메쉬를 그래프로
        학습하므로 재샘플이 없다. 결과 엔진은 ``varying_mesh=True`` 라
        :meth:`predict` 의 기존 형상 가변 분기(``predict_to_mesh`` — 보고
        있는 원본 케이스 메쉬 위 표시)를 그대로 탄다.
        """
        if not self.case_datasets:
            self._fail(
                "케이스 없음",
        ...
        try:
            result = service.build_mesh_gnn_twin_from_cases(
                ...
                max_epochs=int(self.state.nt_mesh_gnn_epochs or 200),
            )
            self.engine = result["engine"]
            names = result["param_names"]
            ...
            multi = f", 다중 출력 {len(fields)}개" if len(fields) > 1 else ""
            summary = (
                f"field(s)='{', '.join(fields)}', 메쉬 GNN (그래프 회귀{multi}) · "
                f"케이스 {result['n_cases']}개 · 입력 파라미터 {len(names)}개 "
    ...
    def predict(self) -> None:
        ...
        try:
            ...
            # 공통 격자는 진실 격자와 달라 오차장은 생략한다(_predict_on_common_grid
            # 가 nt_predicted 만 세운다).
    ...
    def _predict_on_common_grid(self, values: list[float], point: str) -> None:
        ...
        # v5.4 — 공통 격자 예측은 진실 격자와 달라 오차장을 생략한다. 예측이
                ...
                with v3.VExpansionPanelText():
                    ...
                    method_cards = [
                        ...
                        (
                            "mesh_gnn",
                            "메쉬 GNN (그래프 회귀)",
                            "메쉬를 그래프로 직접 학습 · 재샘플 없음 · 원본 격자 예측",
                            "mdi-graph-outline",
                        ),
                        (
                            "gino",
                        ...
                        with v3.VCard(
                            ...
                            with v3.VCardText(classes="py-2 d-flex align-center"):
                                ...
                                with html.Div():
                                    ...
                                    # 하나만 뜨던 문제 보완). 판정은 데이터
                                    # (strategy_report) 기반이라 전략마다
                                    # 다르게 뜬다 — 예: mesh_gnn_rollout 은
                                    # 케이스 세트 자체를 미지원(단일 케이스
                                    # 시계열 전용)이라 이 캡션이 안 뜬다.
                                    html.Div(
                                        "정상 + 비정상(시간축) 모두 지원",
                                        v_show=(
                                            f"nt_strategy_status['{key}'] && "
                    ...
                    # 선택과 무관하게 한 줄만 뜨므로, "위 카드 중 비정상을
                    # 지원하는 전략은 이 데이터를 (μ, t) 로 학습할 수 있다"는
                    # 걸 카드 목록 바로 아래에서 한 번 더, 눈에 띄게 못박는다
                    # (기존 nt_method_hint 는 그대로 두고 보완). 전략 개수는
                    # 계속 늘어나므로(현재 10개, mesh_gnn_rollout 은 단일
                    # 케이스 전용이라 예외) 숫자를 하드코딩하지 않는다 —
                    # 카드마다 뜨는 "정상 + 비정상 모두 지원" 캡션이 실제
                    # 전략별 판정이다.
                    v3.VAlert(
                        "비정상(시간축) 케이스 세트가 로드되었습니다 — 위 카드 "
                        "중 \"정상 + 비정상 모두 지원\" 이라고 표시된 전략은 "
                        "케이스 파라미터(μ)와 시간(t)을 함께 입력으로 "
                        "학습합니다.",
                    ...
                    # Ⓔ 메쉬 GNN (mesh_gnn, Route 2) — 케이스 세트 전용,
                    # 재샘플 없는 메쉬 네이티브 학습.
                    with html.Div(v_show=("nt_model_method === 'mesh_gnn'",)):
                        with html.Div(v_show=("!nt_case_mode",)):
                            self._tip_row(
                                "케이스 세트 전용",
                                "메쉬 GNN 은 케이스 세트(정상 파라미터 스윕)에서만 "
                        ...
                        with html.Div(v_show=("nt_case_mode",)):
                            self._tip_row(
                                ...
                                "셀 연결)로 바꿔 GCN 이 [좌표, 운전조건] → 필드를 "
                                "학습합니다. 공통 격자 재샘플이 없어 진짜 구멍"
                                "(형상 가변)이 그대로 보존되고, 예측도 보고 있는 "
