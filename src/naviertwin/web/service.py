...
def build_mesh_gnn_twin_from_cases(
    ...
    *,
    ...
    정상(steady) 파라미터 스윕 전용 — 케이스마다 **자기 메쉬를 그래프로**
    (:func:`~naviertwin.core.gnn.case_graph.case_to_graph`) 바꿔 GCN
    (:class:`~naviertwin.core.gnn.gnn_surrogate.case_set_gnn.CaseSetGNN`)을
    학습한다. GeometryFNO 와 달리 공통 격자 재샘플이 없다 — 진짜 구멍이
    뚫린 형상 가변 케이스(예: ``karman_shapes`` 데모)를 원본 격자 그대로
    학습·예측한다. 결과 엔진은 ``training_metadata["varying_mesh"]=True`` 라
    ...
    from naviertwin.core.gnn.gnn_surrogate.case_set_gnn import CaseSetGNN
    from naviertwin.core.preprocessing.group_split import (
        classify_query_split,
        group_train_val_test_split,
    )
