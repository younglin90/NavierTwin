...
## 23. 입력 필드(field-to-field) + 해상도 낮추기 (2026-07-17)

사용자 지적 2건: (a) 출력만 고를 수 있고 입력은 시간뿐이다, (b) 거대한 데이터면
셀값을 메모리가 감당 못 하지 않나 — 보간해서 낮은 해상도로 쓰는 개념이 있지 않나.
둘 다 타당했다.

### (a) 입력 필드 — M4a-2 의 per-point 특징 기계를 함께 얻음
입력 field 는 **(스냅샷, 위치)마다 값이 다르므로** 케이스별 스칼라인 `params` 로는
표현할 수 없다 → 코어에 **per-point 특징**을 추가했다. 이는 §19 에서 M4a-2 로
미뤄둔 "SDF 를 입력 피처로" 와 정확히 같은 기계다.
- `CFDFieldTrainingData.input_features` (n_snap, n_loc, n_in) +
  `prepare_..._from_datasets(input_field_names=...)`.
- `_build_training_matrix`: `X = [coords | input fields | params]` —
  **열 순서가 `predict_at` 과 반드시 일치**해야 한다(주석 명시).
- `predict_at(coords, params, input_features=...)` — 새 입력장으로 예측.
- `predict(params)` 는 학습 입력장을 **파라미터 축으로 보간**해 자기완결적으로
  동작한다(1D 는 선형 보간+범위 밖 클램프, 다차원은 최근접 — 근사임을 문서화).
  덕분에 ③Twin 슬라이더가 수정 없이 그대로 돈다.
- 입력=출력 중복은 코어가 거부(항등 학습 방지), 앱은 UI 실수를 미리 필터.
- ROM 은 파라미터→POD계수 구조라 per-point 입력을 못 받는다 → UI 에서 Physics
  방식일 때만 노출.

**검증이 핵심이었다**: 데모(필라멘트/TG)는 p 와 U 가 **둘 다 (x,t)의 함수**라
입력 field 를 줘도 정보가 늘지 않는다(RMSE 0.233 vs 0.232 — 차이 없음). 그래서
`p = 2·s`(s 는 좌표·시간과 무관한 난수)인 합성 데이터로 검증했다:
- 입력 field 有 검증 RMSE **0.027** vs 無 **0.61** (22배) — 경로가 실제로 쓰임.
- 학습에 전혀 없던 **새 입력장**에서 rel **2.4%** — 진짜 field-to-field 연산자.

### (b) 해상도 낮추기 (coarsen)
메모리 문제 진단: `extract_field_snapshots` 가 **(n_features × n_steps) 전체
행렬을 메모리에 올린다** — 1천만 점 × 100 스텝 = float64 8GB. Physics AI 의
`max_train_points` 는 이 행렬을 만든 **뒤** 부분추출하므로 피크를 못 줄인다.
→ 학습 전에 해상도를 낮추는 것이 유일한 실효 대응.
- `coarsen_dataset(dataset, resolution)` — M4a 의 격자 생성 로직을
  `_uniform_grid_over()` 로 추출해 재사용, 타임스텝마다 `grid.sample(원본)`.
  시계열·벡터장 보존, 원본 불변. 손실 압축임을 UI 캡션에 명시.
- ①Import 에 "해상도 낮추기(대용량 대응)" — 격자 분할 + 적용, 축소 결과 표시.
  케이스 세트는 로드 시 이미 재샘플되므로 거부.
- 실측: 200×200×12스텝 → 스냅샷 행렬 3.84MB → 0.23MB (**16.7× 절감**), 물리
  범위·시간 진화·벡터장 모두 보존. 브라우저: 2,304→441점(5.2×).

### 남은 것
- **SDF 를 입력 피처로**(M4a-2) — 이제 기계는 다 있다. `input_field_names=["sdf"]`
  로 바로 되지만, 재샘플 케이스 세트 경로에 배선하고 검증하는 일이 남았다.
- 입력 field 는 현재 **단일 데이터셋(문제 유형 A)** 경로에만 배선했다 — 케이스
  세트(B) 는 `build_physics_ai_twin_from_cases` 에 같은 인자를 뚫으면 된다.

## 24. 데모 카탈로그 — 계열별 "잘 맞는 데이터" (2026-07-17)

사용자 요청: "좀 더 다양한 케이스 데모 데이터 — 예를 들면 정상상태 형상 다른
여러개". 타당했고, 더 큰 문제를 드러냈다: **데모가 시계열 하나뿐이라 M2~M5 로
만든 기능 대부분을 사용자가 시험할 수 없었다.** 특히 DMD 는 유일한 데모(필라멘트)
가 부적합이라 **늘 66% 오차만 보였다** — 기능이 있어도 없는 것과 같았다.

원칙: **모델 계열마다 "그 계열이 잘 맞는 데이터"를 하나씩 갖춘다.**

| 데모 | 유형 | 목적 |
|---|---|---|
| `filament` | 시계열 | 불연속·비주기 — 어려운 기준 (기존 기본값) |
| `advecting` | 시계열 | 부드러운 준주기 — ROM 표준 케이스 |
| **`waves`** (신규) | 시계열 | 공간모드 2개 × 고유 주파수 → **DMD 가 맞는 유일한 데모** |
| **`sweep`** (신규) | 케이스 세트 | 동일 메쉬 · 운전조건 2개(속도·받음각) 5케이스 — 문제 유형 B |
| **`shapes`** (신규) | 케이스 세트 | 반지름 다른 원기둥 5형상 — **메쉬가 서로 달라** 재샘플+sdf 경로 시연 |

- `make_demo_case_set(kind)` 은 :func:`load_case_set` 과 **같은 형태의 dict** 를
  메모리에서 만든다 → 앱의 케이스 세트 경로를 그대로 타고 파일이 필요 없다.
- ①Import 의 버튼 하나가 **선택 + 설명 캡션 + 로드**로 바뀌고, 시계열이면
  `_set_dataset`, 케이스 세트면 `_set_case_set` 으로 디스패치한다.

검증(브라우저): 5개 전부 로드, 설명 캡션이 선택을 따라가고, `sweep` 은 파라미터
2개·재샘플 없음, `shapes` 는 자동 재샘플(형상 가변 안내). 핵심은 **`waves` +
DMD 재구성 오차 6e-15** (필라멘트 66% 대비) — 주파수도 참값(0.207/0.430) 복원.
테스트 154개 통과(신규 8개, DMD 적합/부적합 대조 포함).
