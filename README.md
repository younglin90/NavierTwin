...
- 데이터셋→트윈 자동 빌드: CFD reader 또는 CSV snapshot 시퀀스에서 metrics/report/checkpoint/engine 생성, manifest에 artifact SHA256/bytes와 parameter contract 기록
...
- Streaming Twin REST: `init → step/observe/observe-batch/observe-line → state` 운영 루프로 실시간 관측·로그 라인 동화 상태를 API에서 조회
...
- 웹 GUI (trame): 데스크톱 GUI 의 브라우저 버전. 워크플로우 Import(데모/CFD 파일/.ntwin 프로젝트 열기) → Analyze(Q/λ₂ + FFT/PSD 차트, field/프로브점 선택) → Reduce(POD + 에너지 스펙트럼 차트 + 모드 3D 시각화) → Model(POD/Randomized POD × RBF/Kriging 학습) → Twin(시간→필드 예측) → Export(스크린샷/CSV/VTK/.ntwin/엔진/HTML 보고서) → Compare(조합별 RMSE/R²/지연시간 순위표) → AI Bench(벤치마크 CFD 데이터셋 → 신경 연산자 학습 → ms 단위 예측)를 `pyvista.trame` 서버사이드 3D 렌더로 제공. 무거운 연산은 비차단(thread executor)으로 실행하며 진행바 표시, 스칼라 분석은 matplotlib 차트 모달, 모델 비교는 데이터테이블로 표시. core 모듈을 그대로 재사용하며 `naviertwin web` 으로 실행 (로컬 단일 사용자)
- 웹 AI Bench: 내장 솔버로 Burgers/Heat 1D (u0→uT), Cavity 2D LBM (u(t)→u(t+Δt)) 연산자 데이터셋을 즉석 생성하거나, 다운로드한 PDEBench(NeurIPS 2022) 포맷 HDF5 를 h5py 로 직접 로드(pdebench 패키지 불필요). FNO1D/FNO2D 자동 선택 학습(손실곡선 차트) → train/test 분할 지표(RMSE·rel.L2) + 단일 예측 지연시간(ms) → 샘플별 참값 vs 예측 비교 차트(1D 곡선 / 2D 필드 3패널)
- 웹 다크 UI/UX: 다크 엔지니어링 테마(커스텀 vuetify3 테마 + `client.Style` 전역 CSS), 브랜드 그라데이션 헤더, 8단계 파이프라인 진행 칩(완료 시 초록·클릭 시 패널 이동), 완료/오류 토스트, 다크 matplotlib 차트. **라이브 학습 진행**: FNO 학습 중 epoch별 진행바 + 유니코드 손실 스파크라인이 실시간 스트리밍(trame `StateQueue` 모니터 — 워커는 큐로만 통신해 스레드 안전)
- 데스크톱 앱(Electron): trame 웹 GUI 를 감싸는 Electron 셸(`electron/`). WSL 안에서 실행되어 WSLg 로 Windows 데스크톱에 창 표시, 서버 자동 기동/종료·스플래시·헬스 폴링. 실행: `cd electron && npm install && ./run.sh` (자세히: `docs/ELECTRON.md`)
...
- FastAPI REST 서버 (`/health`, `/doctor`, `/preflight`, `/reduce`, `/reduce/pod`, `/twin/build`, `/twin/predict`, `/twin/benchmark`, `/twin/package`, `/twin/package/inspect`, `/twin/package/verify`, `/twin/package/accept`, `/twin/stream/*`, `/analytic/*`, `/simulate/lbm_cavity`, `/optimize/bayesian`)
- `/twin/predict` REST는 대용량 field 통합용으로 `return_prediction=false` + `output_path` 조합을 지원해 전체 배열 JSON 반환 없이 CSV/NPY 파일과 shape/preview/latency metadata만 받을 수 있음
...
python scripts/benchmark_native_kernels.py --sizes small medium --format json --output /tmp/naviertwin-native-benchmark.json
naviertwin server --host 0.0.0.0 --port 8000
naviertwin autorefine --iterations 1 --dry-run

# 웹 GUI (trame, 브라우저) — 로컬 단일 사용자
pip install -e ".[web]"            # trame / trame-vtk / trame-vuetify
naviertwin web                      # http://127.0.0.1:8080 자동 열림
naviertwin web --host 0.0.0.0 --port 8080 --no-browser
...
- `GET /health`
- `GET /doctor` — 설치/런타임 환경 진단 (`include_optional=true`로 GUI/API/GPU 선택 의존성 포함)
- `POST /preflight` — CFD 입력 데이터 readiness 점검
- `POST /analytic/couette`
- `POST /analytic/poiseuille_2d`
- `POST /reduce` — `pod`, `incremental_pod`, `mrpod`
- `POST /reduce/pod` — 하위 호환 POD 전용
- `POST /twin/build` — CFD/CSV dataset에서 `TwinEngine` 산출물 생성
- `POST /twin/predict` — 저장/배포된 `TwinEngine`의 파라미터→유동장 예측 (`return_prediction=false`, `output_path`, `output_format=csv|npy`로 대용량 응답 제어)
- `POST /twin/benchmark` — 배포된 `TwinEngine`의 예측 latency/SLO 측정
- `POST /twin/package` — `TwinEngine` 산출물을 고객 전달 ZIP으로 패키징
- `POST /twin/package/inspect` — 전달 ZIP 구성/메타데이터 조회
- `POST /twin/package/verify` — 전달 ZIP 무결성 검증/선택 추출
- `POST /twin/package/accept` — 전달 ZIP 검증·샘플 예측·latency 수락 검사
- `POST /twin/stream/init` — 선형 전이 행렬 기반 `StreamingDigitalTwin` 세션 초기화
- `POST /twin/stream/step` — 세션 forecast 전파
- `POST /twin/stream/observe` — 관측 벡터 EnKF 동화
- `POST /twin/stream/observe-batch` — 관측 벡터 배치 동화
- `POST /twin/stream/observe-line` — CSV/log 라인에서 관측값 컬럼을 파싱해 동화
- `GET /twin/stream/state` — 현재 estimate/uncertainty/history 조회
...
├── api/                      # FastAPI REST 서버
├── web/                      # 웹 GUI (trame) — render / service / app
...
- 트윈 전달 ZIP 원샷 acceptance smoke(delivery latency_slo 기본 적용): `naviertwin accept-twin-package --package /tmp/naviertwin-twin.zip --extract-to /tmp/naviertwin-accepted --output /tmp/naviertwin-acceptance.json --summary-output /tmp/naviertwin-acceptance.md --json`
