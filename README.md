...
- 모델 패널: Kriging/RBF + 신경 연산자(FNO/TFNO/DeepONet/UNet/WNO) + Active Learning 후보 추천
- Twin 패널: 파라미터 예측, 저장/로드, Surrogate/Bayesian 기반 inverse-design 최적화
- Export 패널: `.ntwin`, VTK/CSV, 고객 보고서, ONNX/TorchScript 모델 아티팩트
- Explain 탭: Kernel SHAP feature attribution, Attention weight matrix/top-k token viz
- Tools 메뉴: benchmark smoke, pipeline-demo 산출물 생성, FastAPI 서버 start/stop
