"""Build and inspect a NavierTwin wheel artifact.

The check intentionally avoids dependency installation and network access. It
builds a wheel from the current tree, then validates metadata and runtime assets
that must be present for a customer install.
"""
...
REQUIRED_WHEEL_FILES = [
    "naviertwin/gui/styles/dark_theme.qss",
    "naviertwin/gui/styles/i18n/ko.json",
    "naviertwin/gui/styles/i18n/en.json",
]
...
def _build_wheel(outdir: Path) -> Path:
    ...
    if len(wheels) != 1:
        raise RuntimeError(f"expected exactly one naviertwin wheel, got {wheels}")
    return wheels[0]
...
def _validate_wheel(wheel: Path) -> None:
    with zipfile.ZipFile(wheel) as zf:
        ...
        missing_meta = [
            snippet for snippet in REQUIRED_METADATA_SNIPPETS if snippet not in metadata
        ]
        if missing_meta:
            raise RuntimeError(f"wheel metadata missing snippets: {missing_meta}")
        message = Parser().parsestr(metadata)
        if message.get("Name") != "naviertwin":
            raise RuntimeError(f"wheel Name mismatch: {message.get('Name')}")
        if message.get("Requires-Python") != ">=3.10":
            raise RuntimeError(f"wheel Requires-Python mismatch: {message.get('Requires-Python')}")
        if message.get("License-Expression") != "MIT":
            raise RuntimeError(
                f"wheel License-Expression mismatch: {message.get('License-Expression')}"
            )
        ...
        for snippet in required_requires_dist_snippets:
            if snippet not in requires_dist:
                raise RuntimeError(
                    f"wheel Requires-Dist missing '{snippet}': {requires_dist}"
                )

        entry_points_names = [name for name in names if name.endswith(".dist-info/entry_points.txt")]
        if len(entry_points_names) != 1:
            raise RuntimeError(f"expected one entry_points.txt, got {entry_points_names}")
        entry_points = zf.read(entry_points_names[0]).decode("utf-8")
        parser = configparser.ConfigParser()
        parser.read_string(entry_points)
        sections = set(parser.sections())
        if sections != {"console_scripts"}:
            raise RuntimeError(f"wheel entry_points sections mismatch: {sections}")
        ...
        if console_scripts != {"naviertwin": "naviertwin.main:main"}:
            raise RuntimeError(f"wheel console_scripts mismatch: {console_scripts}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--outdir",
        default="/tmp/naviertwin-wheel-smoke",
        help="Directory for temporary wheel output.",
    )
    parser.add_argument(
        "--install-smoke",
        action="store_true",
        help="Install the built wheel into a temporary venv and run naviertwin --help.",
    )
    ...
    print(f"wheel smoke passed: {wheel}", flush=True)
    return 0
