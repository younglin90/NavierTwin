...
REST 통합에서 field가 큰 경우에는 ``POST /twin/predict`` 요청에
``return_prediction=false``, ``output_path``, ``output_format=csv|npy`` 를 지정해
전체 배열 JSON 직렬화 없이 shape, preview, one-shot latency와 파일 경로만 받을 수 있습니다.
...
- **지원 번들 점검**: 받은 support-bundle ZIP 선택 → metadata/artifact hash와 MANIFEST 무결성 확인
