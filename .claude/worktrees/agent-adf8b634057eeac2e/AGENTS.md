# NavierTwin — Codex 지시서

## 프로젝트 개요
CFD 후처리 결과 → AI/ROM/Operator Learning → 디지털 트윈 변환 Windows 데스크톱 툴.
MIT 오픈소스(상업적 이용 가능). Python 3.10+.
GPL/LGPL·무거운 의존성은 optional extra 또는 격리 경로로 분리한다.
...
## 현재 단계
`ROADMAP.md` 참조. 현재 패키지 버전은 `pyproject.toml`/`src/naviertwin/__init__.py` 기준으로 관리한다.
